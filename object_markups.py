...
def get_inline_markup(width, length, *args, isonetime=False):
    markup = telebot.types.InlineKeyboardMarkup(row_width=width)
    # print('srgs =',args, len(args), length)
    args_iterator = 0
    buts = []
...
# получаем на вход id нажатой кнопки и объект игрока
# делаем загрузку всех временных кнопок
# если к кнопке\отцовской кнопке приписано действие и оно есть в массиве действий, то выполняем его
# ловим и обрабатываем выходной сигнал, если он 0 - изменился текст, меняем текст в parent_but
# если 1, то кнопка финальная,(и поиск по родительским кнопкам не проводится, если присутствует action,
#  и срабатывает вылет на form_b_data ) выводим только back
